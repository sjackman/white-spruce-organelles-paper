...
We ran the gap-filling application Sealer (Paulino et al., in review; options -v -j 12 -b 30G -B 300 -F 700 -k 18 to 108, step 6) on the ABySS vX.X.XX (Simpson et al. 2009) assembly of the chloroplast genome, closing 5 out of the remaining 7 gaps, with a resulting assembly consisting of two large (\~50 and 70 kbp) scaftigs. Given the plastid genome size (123,266 bp), we opted to manually finish the assembly with Consed v.20 (Gordon D, Abajian C, Green P: Consed: a graphical tool for sequence finishing. Genome Research 8:195-202 (1998)). Briefly, we loaded the resulting gap-filled assembly into Consed and imported Pacific Biosciences (PacBio, data description NEEDED XX) reads 500 bp and larger (n=9204 reads) into the assembly and aligned them to the choloroplast genome with cross\_match (Green P. http://www.phrap.org/ NO PUB XX) from within Consed. For each scaftig ends, 6 PacBio reads were pulled out and assembled de novo using the mini-assembly feature in Consed. Cross\_match alignments of those resulting contigs to the chloroplast assembly were used to merge the two scaftigs and confirm that the complete sequence was obtained (ie. circular genome). In a subsequent step, 7,742 Illumina HiSeq were imported and aligned onto assembly following the same same procedure as above. We only used this limited 7,742 HiSeq reads to make finishing more manageable. These reads were selected on the basis of BWA (REFXX bwa-0.7.5a) alignment to our draft chloroplast genome, focusing on regions that would benefit from read import by restricting our search using samtools (REFXX samtools-0.1.18/samtools view -X align.bam) (regions with ambiguity and regions covered by PacBio reads exclusively). As described for the PacBio data, subset of Illumina reads were pulled out, mini-assembled with Phrap (Green P. http://www.phrap.org/ NO PUB XX) and the resulting contigs re-merged to correct bases in gaps filled by PacBio only (1 gap and sequence at edges confirming the circular topology). The starting base was chosen using the Norway spruce plastid genome sequence deposited in NCBI-Genbank (gi|523420621|ref|NC\_021456.1| Picea abies chloroplast complete genome). Our assembly was further polished using the Genome Analysis Toolkit (GATK XXREF, version 2.8-1-g932cd3a -T FastaAlternateReferenceMaker).
...
The white spruce mitochondrial genome was assembled as described above, resulting in 71 scaffolds (XX STATS?). We explored the possibility to merge scaffolds by running our gap-filling application Sealer in a combinatorial fashion, targeting every combination of sequences for scaffold gap closure. This approach yielded 61 scaffolds, which we used as input to the LINKS scaffolder (Warren et al., in review; v1.1 options: -k 15 -t 1 -l 3 -r 0.4, 19 iterations –d 500 to 6000:250bp) in conjunction with long PacBio reads, further decreasing the number of scaffolds to 58. White spruce PG29 Konnector pseudoreads were aligned against the 58 LINKS scaffolds (bwa-0.7.5a/bwa mem -t4 -a multimap) and we created links between 2 scaffolds when reads reportedly aligned within the 1000 bp edges of any 2 scaffolds. Alignments were no necessarily split, nor primary (in some cases, the primary alignment was buried in a large scaffolds, but secondary alignments suggest redundant sequences flanking a scaffold "pair"). We modified LINKS to read the resulting .sam alignment file and link scaffold satisfying this criteria (unpublished; option ./LINKS-sam -e 0.9 -a 0.5), bringing the final number of scaffolds to 38. We validated the merges using MPET reads. The white spruce MPET libraries used for validation are presented in \[Birol et al. 2013\] and available from the dnanexus repository (http://sra.dnanexus.com/studies/SRP014489). In brief, MPET reads from three fragment size libraries (5k, 8k, 12kbp) were aligned to the 38-scaffold assembly with bwa mem (REFXX; v0.7.10-r789 -t 16) and the resulting alignments parsed with custom PERL scripts. A summary of this validation is presented in supplemental table XX. Automated gap-closing was performed with Sealer (v1.0 options -j 12 -B 1000 -F 700 -P10 -k96 and -k80 using Bloom filters built from the entire white spruce PG29 read data set (Warren et al. 2015 PLANT JOURNAL REFXX)) and closed 55 of the 182 total gaps (30.2%). We polished the gap-filled assembly using GATK and a single HiSeq white spruce sequence lane (SRA ID?), as described for the chloroplast genome.
